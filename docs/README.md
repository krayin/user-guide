---
home: true
heroText: Krayin User Documentation 
tagline: A Comprehensive User Guide to Seamlessly Navigate Krayin.
actionText: Get Started →
actionLink: /2.x/introduction/

features:
- title: Leads and Pipeline
  details: Learn how to create leads manage lead pipeline for Krayin.

- title: Activites
  details: Learn how to create and manage Activites for Krayin.  

- title: Quotes
  details: Learn how to create and manage quotes for Krayin.  

footer: Copyright © 2025 Krayin
---